...
class Rosmaster(object):
    ...
    def __init__(self, bot_type=1, com="/dev/myserial", delay=.002, debug=True):
        # com = "COM30"
        # com="/dev/ttyTHS1"
        # com="/dev/ttyUSB0"
        # com="/dev/ttyAMA0"
        ...
        self.FUNC_DRIVE = 0x16
        self.FUNC_REPORT_GPS = 0x17
        self.FUNC_MANIPULATOR = 0x18
    ...
    def __parse_data(self, ext_type, ext_data):
        ...
        if ext_type == self.FUNC_REPORT_SPEED:
            print("Inside the FUNC_REPORT_SPEED")
            # print(ext_data)
        ...
        # Encoder data on all four wheels
            ...
            print("Inside the FUNC_REPORT_ENCODER")
            ...
            if ext_type == self.FUNC_UART_SERVO:
                ...
                if self.__debug:
                    print("FUNC_SET_MOTOR_PID:", self._pid_index, [self.kp1, self.ki1, self._kd1])
                ...
                self.__kd1 = struct.unpack('h', bytearray(ext_data[5:7]))[0]
                if self.__debug:
                    print("FUNC_SET_YAW_PID:", self._pid_index, [self.kp1, self.ki1, self._kd1])

            elif ext_type == self.FUNC_REPORT_GPS:
                print("Inside the FUNC_REPORT_GPS")
                self.__latitude = struct.unpack('f', bytearray(ext_data[0:4]))[0]
                self.__longitude = struct.unpack('f', bytearray(ext_data[4:8]))[0]
                self.__altitude = struct.unpack('f', bytearray(ext_data[8:12]))[0]
                self.__speed = struct.unpack('f', bytearray(ext_data[12:16]))[0]

                if self.__debug:
                    print(f"GPS Data: Latitude={self.__latitude}, Longitude={self.__longitude}, "
                        f"Altitude={self.__altitude}, Speed={self.__speed}")   
            ...
            elif ext_type == self.FUNC_SET_bot_TYPE:
                bot_type = struct.unpack('B', bytearray(ext_data[0:1]))[0]
                self.__read_bot_type = bot_type
        ...
        while True:
            ...
            if head1 == self.__HEAD:
                ...
                if head2 == self.__DEVICE_ID - 1:
                    ...
                    print("ext_len:", ext_len, "ext_type:", ext_type, "ext_data:", ext_data)
                    rx_check_num = bytearray(self.ser.read())[0]
    ...
    def set_pwm_servo_all(self, angle_s1, angle_s2, angle_s3, angle_s4):
        try:
            ...
            if angle_s4 < 0 or angle_s4 > 180:
                angle_s4 = 255
            cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_PWM_SERVO_ALL, \
    ...
    # Control the bot forward, backward, left, right and other movements.
    ...
    def set_bot_run(self, state, speed, adjust=False):
        try:
            ...
            cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_bot_RUN, \
            ...
            print('---set_bot_run error!---')
            pass

    # bot movement control
    def set_drive_mtr(self, drive_mtr, swerve_mtr, drive_mtr_pwm, swerve_mtr_pwm, swift_turn, bogie_mtr):
        """
        Sends control data packets to the ESP32.
    
        Parameters:
            drive_mtr (int): 1 or -1 for drive motor direction.
            swerve_mtr (int): 1 or -1 for swerve motor direction.
            drive_mtr_pwm (int): PWM value (0-255) for drive motor speed.
            swerve_mtr_pwm (int): PWM value (0-255) for swerve motor speed.
            swift_turn (int): 0 or 1 indicating swift turn state.
            bogie_mtr (int): 0 or 1 indicating bogie motor state.
        """
        try:
            # Pack the parameters into a command
            drive_mtr_byte = bytearray(struct.pack('h', int(drive_mtr*1000)))
            swerve_mtr_byte = bytearray(struct.pack('h', int(swerve_mtr*1000)))
            drive_mtr_pwm_byte = bytearray(struct.pack('h', int(drive_mtr_pwm)))
            swerve_mtr_pwm_byte = bytearray(struct.pack('h', int(swerve_mtr_pwm)))
            swift_turn_byte = bytearray(struct.pack('h', int(swift_turn)))
            bogie_mtr_byte = bytearray(struct.pack('h', int(bogie_mtr)))

            cmd = [
                self.__HEAD, self.__DEVICE_ID, 0x00, self.FUNC_DRIVE,self.__bot_TYPE,      
                drive_mtr_byte[0],drive_mtr_byte[1] ,swerve_mtr_byte[0],swerve_mtr_byte[1],drive_mtr_pwm_byte[0],drive_mtr_pwm_byte[1], swerve_mtr_pwm_byte[0],
                swerve_mtr_pwm_byte[1],swift_turn_byte[0],swift_turn_byte[1], bogie_mtr_byte[0], bogie_mtr_byte[1]
            ]
            ...
            if self.__debug:
                print("ESP32 Control Command:", cmd)

            # Introduce delay to ensure data transmission
            time.sleep(self.__delay_time)

        except Exception as e:
            print('---set_esp32_control error!---', str(e))
            pass
  
    def set_manipulator_mtr(self, base,angle,arm,claw,pwm,forward,backward):
        """
        Sends control data packets to the ESP32.
        ...
        try:
            # Pack the parameters into a command
            base_byte = bytearray(struct.pack('h', int(base)))
            angle_byte = bytearray(struct.pack('h', int(angle)))
            arm_byte = bytearray(struct.pack('h', int(arm)))
            claw_byte = bytearray(struct.pack('h', int(claw)))
            pwm_byte = bytearray(struct.pack('h', int(pwm)))
            forward_byte = bytearray(struct.pack('h', int(forward)))
            backward_byte = bytearray(struct.pack('h', int(backward)))

            cmd = [
                self.__HEAD, self.__DEVICE_ID, 0x00, self.FUNC_MANIPULATOR,self.__bot_TYPE,      
                base_byte[0],base_byte[1] ,angle_byte[0],angle_byte[1],arm_byte[0],arm_byte[1], claw_byte[0],
                claw_byte[1],pwm_byte[0],pwm_byte[1] ,forward_byte[0],forward_byte[1], backward_byte[0], backward_byte[1]
            ]
    ...
    def set_bot_motion(self, v_x, v_y, v_z):
        try:
            ...
            print('---set_bot_motion error!---')
            pass

    # PID parameter control will affect the set_bot_motion function to control the speed change of the bot.  This parameter is optional by default.  
    # KP ki kd = [0, 10.00]  
    # forever=True for permanent, =False for temporary.  
    ...
    def set_bot_type(self, bot_type):
        if str(bot_type).isdigit():
            ...
            cmd = [self._HEAD, self.DEVICE_ID, 0x00, self.FUNC_SET_bot_TYPE, self._bot_TYPE, 0x5F]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xff
            cmd.append(checksum)
            self.ser.write(cmd)
            ...
            print("set_bot_type input invalid")
    ...
    def set_uart_servo(self, servo_id, pulse_value, run_time=500):
        try:
            ...
            cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_UART_SERVO, \
    ...
    def set_uart_servo_angle_array(self, angle_s=[90, 90, 90, 90, 90, 180], run_time=500):
        try:
            ...
            if 0 <= angle_s[0] <= 180 and 0 <= angle_s[1] <= 180 and 0 <= angle_s[2] <= 180 and \
                ...
                r_time = bytearray(struct.pack('h', int(run_time)))
                cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_ARM_CTRL, \
    ...
    # Set the default Angle of akerman type (R2) bot front wheel, Angle =[60, 120]
    # forever=True for permanent, =False for temporary.
    # Since permanent storage needs to be written into the chip flash, which takes a long time to operate, delay is added to avoid packet loss caused by MCU.  
    # Temporary effect fast response, single effective, data will not be maintained after restarting the single chip
    def set_akm_default_angle(self, angle, forever=False):
        try:
            ...
            cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_AKM_DEF_ANGLE, id, int(angle), state]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xff
            cmd.append(checksum)
            self.ser.write(cmd)
    ...
    # Control the steering Angle of ackman type (R2) bot relative to the default Angle, negative for left and positive for right, Angle =[-45, 45]
    # ctrl_bot=False, only control the steering gear Angle, =True, control the steering gear Angle and modify the speed of the left and right motors.
    def set_akm_steering_angle(self, angle, ctrl_bot=False):
        try:
            ...
            cmd = [self._HEAD, self._DEVICE_ID, 0x00, self.FUNC_AKM_STEER_ANGLE, id, int(angle)&0xFF]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xff
            cmd.append(checksum)
            self.ser.write(cmd)
    ...
    # Reset the bot flash saved data, restore the factory default value
    ...
    # Reset bot status, including parking, lights off, buzzer off
    def reset_bot_state(self):
        try:
            ...
            self.ser.write(cmd)
            if self.__debug:
                print("reset_bot_state:", cmd)
            time.sleep(self.__delay_time)
        except:
            print('---reset_bot_state error!---')
            pass
    ...
    # Read the default angle of the front wheel servo of Ackermann type (R2) car.
    def get_akm_default_angle(self):
        if not self.__akm_readed_angle:
            self._request_data(self.FUNC_AKM_DEF_ANGLE, self._AKM_SERVO_ID)
            akm_count = 0
            while True:
    ...
    def get_imu_attitude_data(self, ToAngle=True):
        if ToAngle:
            ...
            roll, pitch, yaw = self._roll, self.pitch, self._yaw
        # self._roll, self.pitch, self._yaw = 0.0, 0.0, 0.0
        return roll, pitch, yaw

    # Get the bot speed, val_vx, val_vy, val_vz
    def get_motion_data(self):
        val_vx = self.__vx
        val_vy = self.__vy
        val_vz = self.__vz
        # self._vx, self.vy, self._vz = 0.0, 0.0, 0.0
        return val_vx, val_vy, val_vz

    def get_gps_data(self):
        return self.__latitude, self.__longitude, self.__altitude, self.__speed 
    ...
    def get_motor_encoder(self):
        ...
        # self._encoder_m1, self.encoder_m2, self.encoder_m3, self._encoder_m4 = 0, 0, 0, 0
        return m1, m2, m3, m4
    ...
    # Gets the current bot type from machine
    def get_bot_type_from_machine(self):
        self.__request_data(self.FUNC_SET_bot_TYPE)
        for i in range(0, 20):
            if self.__read_bot_type != 0:
                bot_type = self.__read_bot_type
                self.__read_bot_type = 0
                return bot_type
            time.sleep(.001)
        return -1
...
if __name__ == '__main__':
    ...
    if device == 'Windows':
        ...
        while True:
            ...
            try:
                ...
                print(f"Error while creating Rosmaster object: {e}")
                if com_index > 256:
                    print("-----------------------No COM Open--------------------------")
                    exit(0)
                continue
    ...
    # bot.set_bot_type(4)
    # time.sleep(.1)
    # bot_type = bot.get_bot_type_from_machine()
    ...
    try:
        while True:
            ...
            # bot.set_bot_motion(-1, 0.045, 30)
            # bot.set_drive_mtr(-1,0,100,100,1,2)
            bot.set_manipulator_mtr(1,1,1,1,155,0,1)
            # if bot.ser.in_waiting > 0:  # Check if there's data to read
            #     data = bot.ser.readline().decode('utf-8').strip()  # Read one line, decode it, and strip extra spaces/newlines
            #     print(f"R: {data}")
            # time.sleep(.1)
